# Plugin E2E Tests Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite `tests/e2e/plugin-flow.test.ts` to cover all user stories (evaluate, check, regression, report, approve, error paths) through the Copilot plugin.

**Architecture:** Single test file with shared helpers (`isCopilotAvailable`, `invokeCopilotWithPlugin`, fixture copiers). Each `it()` block is a self-contained user story test. Assertions are tiered: file artifacts (primary), stdout patterns (secondary), exit codes (tertiary/best-effort).

**Tech Stack:** Vitest, `node:child_process` (execFileSync), `node:fs`, real Copilot CLI with plugin installed.

**Spec:** `docs/superpowers/specs/2026-03-15-plugin-e2e-tests-design.md`

---

## Chunk 1: Rewrite plugin-flow.test.ts

### Task 1: Write helpers and plugin lifecycle

**Files:**
- Rewrite: `tests/e2e/plugin-flow.test.ts`

- [ ] **Step 1: Write the helpers and describe shell**

Replace the entire file with the new structure. This step writes the imports, constants, helpers, plugin lifecycle (`beforeAll`/`afterAll`/`afterEach`), and the `describe` shell with `skipIf`. No test bodies yet.
...
/** Copy full greeter fixture (SKILL.md + evals/ + snapshots/). */
function copyFullFixture(destDir: string): void {
  fs.cpSync(GREETER_SRC, destDir, { recursive: true });
}
...
Run: `npx tsc --noEmit`
Expected: No errors related to `plugin-flow.test.ts`.

- [ ] **Step 3: Commit**

```bash
git add tests/e2e/plugin-flow.test.ts
git commit -m "refactor(e2e): scaffold plugin user story tests with helpers"
```

---

### Task 2: US1 — Evaluate flow

**Files:**
- Modify: `tests/e2e/plugin-flow.test.ts`

- [ ] **Step 1: Add the US1 test inside the `describe` block**

Add after the `afterAll` block, inside `describe.skipIf(!copilotAvailable)`:
...
```bash
git add tests/e2e/plugin-flow.test.ts
git commit -m "test(e2e): US1 — evaluate flow through plugin"
```

---

### Task 3: US2 — Check flow (pass)

**Files:**
- Modify: `tests/e2e/plugin-flow.test.ts`

- [ ] **Step 1: Add the US2 test**
...
```bash
git add tests/e2e/plugin-flow.test.ts
git commit -m "test(e2e): US2 — check pass flow through plugin"
```

---

### Task 4: US2b — Check flow (regression detected)

**Files:**
- Modify: `tests/e2e/plugin-flow.test.ts`

- [ ] **Step 1: Add the US2b test**
...
```bash
git add tests/e2e/plugin-flow.test.ts
git commit -m "test(e2e): US2b — regression detection through plugin"
```

---

### Task 5: US3 — Report flow

**Files:**
- Modify: `tests/e2e/plugin-flow.test.ts`

- [ ] **Step 1: Add the US3 test**
...
```bash
git add tests/e2e/plugin-flow.test.ts
git commit -m "test(e2e): US3 — HTML report generation through plugin"
```

---

### Task 6: US4 — Approve flow

**Files:**
- Modify: `tests/e2e/plugin-flow.test.ts`

- [ ] **Step 1: Add the US4 test**
...
```bash
git add tests/e2e/plugin-flow.test.ts
git commit -m "test(e2e): US4 — approve flow through plugin"
```

---

### Task 7: Error paths (US-ERR1 and US-ERR2)

**Files:**
- Modify: `tests/e2e/plugin-flow.test.ts`

- [ ] **Step 1: Add both error path tests**
...
it('US-ERR2: check with no baselines — reports error', () => {
  ...
  if (fs.existsSync(snapshotsDir)) {
    ...
    // If snapshots dir was created, it should be empty (no corrupt snapshots)
...
```bash
git add tests/e2e/plugin-flow.test.ts
git commit -m "test(e2e): US-ERR1 and US-ERR2 — error path tests through plugin"
```
...
Run: `npm test`
Expected: All existing unit/integration tests pass. Plugin e2e tests are excluded from `npm test` (separate vitest config).

- [ ] **Step 3: Dry-run the e2e tests (if Copilot available)**

Run: `npm run test:e2e`
Expected: If Copilot is available and authenticated, tests run. Some may be slow (300s timeout). If Copilot is not available, all tests skip cleanly.

- [ ] **Step 4: Final commit (if any lint/type fixes needed)**

```bash
git add tests/e2e/plugin-flow.test.ts
git commit -m "test(e2e): finalize plugin user story tests"
```
